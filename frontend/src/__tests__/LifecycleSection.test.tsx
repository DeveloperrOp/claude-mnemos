    ...
    "translation",
    {
      settings: {
        ...
        section: {
          lifecycle: {
            title: "Lifecycle",
            auto_stale_days: "Auto-stale (days)",
            auto_archive: "Auto-archive stale pages",
          },
        },
      },
    },
...
describe("LifecycleSection", () => {
  ...
  it("toggle auto_archive enables Save and PATCHes", async () => {
    vi.mocked(apiClient.get).mockResolvedValueOnce({ data: FULL });
    vi.mocked(apiClient.patch).mockResolvedValueOnce({
      data: {
        ...FULL,
        lifecycle: { auto_stale_days: 90, auto_archive: true },
      },
    });
    wrap(<LifecycleSection slug="p1" />);
    await waitFor(() =>
