...
# Windows VERSIONINFO resource, generated by installer/set_version.py on CI
# tag builds (absent in dev trees → exe ships without a version resource,
# same as before). Windows-only: PyInstaller ignores `version=` elsewhere.
_version_file = Path(SPECPATH) / "version_info.txt"
_version_res = str(_version_file) if sys.platform == "win32" and _version_file.exists() else None
