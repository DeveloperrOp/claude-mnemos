...
def test_get_dead_letter_unknown_returns_404(
    ...
    _daemon, client = daemon_with_two_vaults
    r = client.get("/api/dead-letter/00000000-0000-0000-0000-000000000000")
...
def test_retry_routes_to_correct_runtime(
    ...
    r = client.post(f"/api/dead-letter/{alpha_job['id']}/retry")
    assert r.status_code == 200, r.text
    body = r.json()
...
def test_dismiss_routes_to_correct_runtime(
    ...
    jobs = list_r.json()["jobs"]
    beta_job = next(j for j in jobs if j["project_name"] == "beta")
    r = client.delete(f"/api/dead-letter/{beta_job['id']}")
...
def test_dismiss_unknown_returns_404(
    ...
    _daemon, client = daemon_with_two_vaults
    r = client.delete("/api/dead-letter/00000000-0000-0000-0000-000000000000")
    assert r.status_code == 404
...
def test_list_dead_letter_cross_vault_pagination(
    ...
    page1 = client.get("/api/dead-letter?limit=4&offset=0").json()["jobs"]
    page2 = client.get("/api/dead-letter?limit=4&offset=4").json()["jobs"]
    page3 = client.get("/api/dead-letter?limit=4&offset=8").json()["jobs"]
