"""PreCompact hook (Feature A): cwd → project-map → POST /jobs with extract=False.

End-to-end runs of ``hooks/pre_compact.py`` as a subprocess. Each test isolates
...
HOOK = Path(__file__).resolve().parents[2] / "hooks" / "pre_compact.py"
